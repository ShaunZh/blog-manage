<template>
  <div class="notebook-setting">
    <div class="go-to-blog pt-4 pb-4 text-center">
      <a class="px-4 py-2 ">回到首页</a>
    </div>
    <div class="add-notebook py-2">
      <div class="pl-3 add-notebook-btn " @click.stop="isDispAddNotebook = true">+ 新建文集</div>
      <form class="dispAddNotebook pl-3 my-1" v-show="isDispAddNotebook === true">
        <div class="form-group my-0">
          <input type="text" class="notebook-input" placeholder="请输入文集名" autofocus v-model="newNotebookName">
        </div>
        <div class="form-group">
          <button type="submit" @click.stop="submitAddNotebook" class="add-btn btn btn-outline-success px-3 py-1 ">提交</button>
          <button type="submit" @click.stop="isDispAddNotebook = false" class="cancel-btn ml-1 btn">取消</button>
        </div>
      </form>
    </div>
    <ul class="notebooksList p-0">
      <li class="item row mx-0 text-left py-3" :class="{active: item.id === notebooksListBk[currentNotebook].id}"
          v-for="(item, index) in notebooksListBk" @click.stop="changeNotebook(index)">
        <div class="col-9" >{{item.name}}</div>
        <div class="col-3" v-show="item.id === notebooksListBk[currentNotebook].id">
        ...
        <ul class="setting-info p-0 bg-light" v-show="item.isDispSetting">
          <li class="modify px-3 py-2 ">
            ...
            <span href="#" @mousedown.stop="modifyNotebook(index)">修改文集</span>
          </li>
          <li class="delete  px-3 py-2">
            ...
            <span href="#" @mousedown.stop="deleteNotebook(index)">删除文集</span>
          </li>
        </ul>
      </li>
    </ul>
...
export default {
  ...
  data() {
    return {
      newNotebookName: '',
      isDispAddNotebook: false,
      currentNotebook: 0,
      notebooksListBk: null,
    };
  },
  watch: {
    isDispAddNotebook(newValue) {
      if (!newValue) {
        this.newNotebookName = '';
      }
    },
    notebooksList(newValue) {
      this.notebooksListBk = JSON.parse(JSON.stringify(newValue));
      this.notebooksListBk = this.notebooksListBk.map(item =>
        Object.assign({}, item, { isDispSetting: false }));
    },
  },
  created() {
    this.notebooksListBk = JSON.parse(JSON.stringify(this.notebooksList));
    this.notebooksListBk = this.notebooksListBk.map(item =>
      Object.assign({}, item, { isDispSetting: false }));
  },
  methods: {
      ...
      axios({
        ...
        .then((response) => {
          if (response.data.status === 200) {
            this.$emit('getNewNotebook', response.data.data);
            this.isDispAddNotebook = false;
          }
        })
        .catch((error) => {
    ...
    /**
     * @description 隐藏addNotebook设置
     * @param
     * @return
     */
    hiddenNotebookSetting() {
    },
    /**
     * @description 删除文集
     * @param
    ...
    deleteNotebook(index) {
      this.$confirm(`此操作将永久删除《${this.notebooksList[index].name}》, 是否继续?`, '警告', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      }).then(() => {
        this.$emit('deleteNotebook', index);
      }).catch(() => {
        // this.$message({
        //   type: 'info',
        //   message: '已取消删除',
    ...
    modifyNotebook(index) {
      this.$prompt('请输入新的文集名', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        inputPlaceholder: this.notebooksList[index].name,
      }).then(({ value }) => {
        this.$emit('updateNotebook', index, value);
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '取消输入',
    ...
    changeNotebook(index) {
      this.currentNotebook = index;
      this.$emit('changeNotebook', index);
    },
    open() {
    },
  },
