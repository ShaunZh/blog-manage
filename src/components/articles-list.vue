<template>
  <div class="article-list mx-1">
    <div class="add-article row m-0 no-gutters" @click.stop="submitAddArticle(1)">
    ...
    <ul class="body p-0" v-if="articlesList.length">
      <li class="row  py-4 px-4 item no-gutters "
          :class="{'active': item.id === articlesList[activeIndex].id}"
          v-for="(item, index) in articlesList"
          @click.stop="switchArticle($event, index)">
        <!--文章发布状态-->
        <div class="col-2 text-left">
          <i class="icon-publish" v-if="item.isPublish">
            <svg version="1.1"  width="20" height="25" viewBox="0 0 500 1000" enable-background="new 0 0 500 1000" xml:space="preserve">
              <image id="image0" width="20" height="25" x="0" y="0"
    xlink:href="data:image/png;base64,
...
v787bFjR0qcViwAAACV0RVh0ZGF0ZTpjcmVhdGUAMjAxOC0wNC0zMFQyMzoxMTo1MCswODowMIZ0
Fb8AAAAldEVYdGRhdGU6bW9kaWZ5ADIwMTgtMDQtMzBUMjM6MTE6NTArMDg6MDD3Ka0DAAAAAElF
TkSuQmCC" /></svg>
          </i>
        ...
        <div class="col-8">
          ...
          <!--文章简介-->
          <!--<div class="col-2" v-show="item.id === articlesList[activeIndex].id">-->
          <!--</div>-->
        </div>

        <!--设置图标-->
        <div class="col-2 text-right">
          <svg viewBox="0 0 1024 1024" width="15" height="15" v-show="item.id === articlesList[activeIndex].id"
          ...
          <ul class="setting-info p-0 bg-light" v-show="item.isDispSetting">
            <li class="modify px-3 py-2 ">
              ...
              <span href="#" @mousedown.stop="modifyTag(index)">设为私密</span>
            </li>
            ...
            <li class="delete px-3 py-2 ">
              ...
              <span href="#" @mousedown.stop="deleteArticle(index)">删除文集</span>
            </li>
          </ul>
        </div>
      </li>
    </ul>
    <div class="add-on-down row m-0 no-gutters" @click.stop="submitAddArticle(-1)">
      <p class="py-3 m-0 px-3 mx-3">
...
export default {
  ...
  methods: {
     ...
     * @return
     */
    submitAddArticle(sort) {
      this.$emit('submitAddArticle', false, sort);  // false 表示的是未发表状态
    },
    /**
     * @description 切换文章
     * @param
     ...
     */
    deleteArticle(index) {
      this.$confirm(`确认删除文章《${this.articlesList[index].title}》，文章将被移动到回收站，您可以在那里恢复它。60天后将被彻底删除。`, '警告', {
